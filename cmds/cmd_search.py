...
class SearchCmd(Command):
    ...
    def fetch_with_retry(self):
            ...
            try:
                ...
                if e.code == 403:
                    ...
                    if attempt < self.MAX_RETRIES:
                        ...
                        raise Exception(
                            "GitHub API 速率限制已用完，请稍后再试（建议等待几分钟后重试）"
                        )
        ...
        if last_exception:
            raise Exception(
                f"经过 {self.MAX_RETRIES} 次重试后仍然失败: {str(last_exception)}"
            )
    ...
    def set_parser(self, p: argparse._SubParsersAction) -> argparse.ArgumentParser:
        ...
        search_parser.add_argument(
            "--clear-cache", action="store_true", help="清理本地缓存文件"
        )
        search_parser.add_argument(
            "--cache-status", action="store_true", help="查看缓存状态信息"
        )
        search_parser.add_argument(
            "--sort",
        ...
        search_parser.add_argument(
            "--limit", type=int, default=None, help="限制显示的包数量"
        )
