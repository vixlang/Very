...
```bash
go install github.com/vixlang/very@latest
```
...
```bash
git clone https://github.com/vixlang/very.git
cd very
go build -o very.exe .
```
...
├── internal/
│   ├── api/                     # 业务逻辑层
│   │   ├── error.go             # 错误类型（NotFound/IOError/Compile/...）
│   │   ├── types.go             # 包名解析（parse_pack_name）
│   │   ├── pkg.go               # 包管理
│   │   ├── tool.go              # 工具管理
│   │   ├── build.go             # 编译/运行/检查
│   │   ├── search.go            # GitHub 搜索
│   │   ├── scaffold.go          # 项目脚手架
│   │   ├── vindex.go            # vindex.toml 读写
│   │   └── vstd.go              # 标准库同步
│   └── cmd/                     # CLI 命令层（cobra）
│       ├── root.go              # 根命令
│       ├── add.go / del.go / list.go / ...
│       └── tool.go              # tool 子命令
├── go.mod
└── go.sum
```
